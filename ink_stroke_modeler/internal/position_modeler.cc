...
absl::StatusOr<int> NumberOfStepsBetweenInputs(
  ...
  if (sampling_params.min_distance_resolution > 0.0) {
    int n_steps_for_distance =
        std::min(std::ceil((end.position - start.position).Magnitude() *
                           sampling_params.min_distance_resolution),
                 static_cast<double>(std::numeric_limits<int>::max()));
    if (n_steps_for_distance > n_steps) {
      n_steps = n_steps_for_distance;
    }
  }
